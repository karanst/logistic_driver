  ...
  _updateProfile() async {
    if (newimage == null) {
      ...
      var request = http.MultipartRequest(
          'POST',
          Uri.parse(
              'https://developmentalphawizz.com/JDX/api/Authentication/editDeliveryProfile'));
      ...
      request.files.add(await http.MultipartFile.fromPath(
          RequestKeys.userImage, newimage?.path ?? ''));
      request.files.add(await http.MultipartFile.fromPath(
          RequestKeys.drivingLicencePhoto, imageFile?.path ?? ''));
      http.StreamedResponse response = await request.send();

      if (response.statusCode == 200) {
        print('_________________-success_______________-');
        final reslut = await response.stream.bytesToString();
        var finalResult = json.decode(reslut);
        print("this is a respoce=======>${finalResult['status']}");
